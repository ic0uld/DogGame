...
void UUploadScoreToSteamLeaderboard::OnDownloadedSteamLeaderboardPersonalScore(LeaderboardScoresDownloaded_t* pCallback, bool bIOFailure)
{
	if (!bIOFailure)
	{
		...
		if (SteamUserStats()->GetDownloadedLeaderboardEntry(pCallback->m_hSteamLeaderboardEntries, 0, &SteamLeaderboardEntry, NULL, 0))
		{
			switch (easyUploadScoreMethod)
			{
				case EEasyLeaderboardUploadScoreMethod::ELUSM_EEasyLeaderboardUploadScoreMethodSubtract:
...
{
	result.Success = static_cast<bool>(pCallback->m_bSuccess);
	result.Score = pCallback->m_nScore;
	result.bScoreChanged = static_cast<bool>(pCallback->m_bScoreChanged);
	result.GlobalRankNew = pCallback->m_nGlobalRankNew;
	result.GlobalRankPrevious = pCallback->m_nGlobalRankPrevious;
